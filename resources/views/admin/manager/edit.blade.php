...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text"  name="mg_name" value="{{$info->mg_name}}">
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-2">初始密码：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text"  name="password" value="{{$info->password}}">
		</div>
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9 skin-minimal">
			<div class="radio-box">
				<input name="mg_status" type="radio" id="sex-1" value="1" @if($info->mg_status == 1) checked @endif>
				<label for="sex-1">正常</label>
			</div>
			<div class="radio-box">
				<input type="radio" id="sex-2" name="mg_status" value="0" @if($info->mg_status == 0) checked @endif>
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="r_id" size="1">
				...
				<option value="{{$k}}" @if($info->r_id === $k) selected @endif>{{$v}}</option>
				@endforeach
			</select>
			</span> </div>
	</div>
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<textarea name="desc" cols="" rows="" class="textarea"  placeholder="说点什么...100个字符以内" dragonfly="true" onKeyUp="$.Huitextarealength(this,100)">{{$info->desc}}</textarea>
